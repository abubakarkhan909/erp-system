/**
 * Rich demo data for every ERP module (Oman / OMR).
 * Safe to re-run: clears previous [DEMO] records first.
 */
import {
  PrismaClient,
  GoldKarat,
...
import { addMoney, calcGoldLine, calcVat, roundMoney, subMoney } from '@jewelry-erp/shared';
...
async function clearDemoData() {
  // Child → parent order for demo-tagged / demo-numbered docs
...
async function ensureUser(
  username: string,
  fullName: string,
  roleCode: 'MANAGER' | 'CASHIER' | 'SALESMAN' | 'ACCOUNTANT',
  password: string,
) {
...
async function postJournal(opts: {
  ...
  sourceType: string;
  sourceId: string;
  lines: Array<{ code: string; debit: string; credit: string; narration?: string; partyType?: PartyType; partyId?: string }>;
  userId: string;
  periodId?: string;
}) {
...
export async function seedDemoData() {
  ...
  for (const p of productDefs) {
    const product = await prisma.product.create({
      data: {
        ...
        brandId: p.brandId,
        productType: p.sku.includes('SCRAP') ? ProductType.RAW_GOLD : ProductType.FINISHED,
  ...
  // For purchase use unit cost style
  const purchNet = '1250.000';
  const purchVat = calcVat(purchNet, 5);
  ...
  // --- Sales ---
  // 1) Cash sale (posted) — Maryam
  const sale1Line = calcGoldLine({
    netWeightGram: '8.200',
    ratePerGram: '27.500',
  ...
  // 2) Credit sale (posted) — Khalid with balance
  const sale2Line = calcGoldLine({
  ...
  const sale2Balance = subMoney(sale2Line.lineTotal, sale2Paid);
  const sale2 = await prisma.saleInvoice.create({
    data: {
      ...
      status: DocumentStatus.POSTED,
      subtotal: addMoney(sale2Line.goldValue, '35.000', '5.000'),
      discount: '10.000',
  ...
  // 3) Installment sale — Aisha
  const sale3Total = '420.000';
  const sale3Vat = calcVat('400.000', 5);
  ...
  // Sale return (posted) against sale1 partial conceptually
  ...
  // VAT snapshot (previous month example)
  ...
  // Low-stock alert product already at min — mark damaged sample movement
  await prisma.stockMovement.create({
    data: {
      productId: band24.id,
...
// Allow standalone run: pnpm exec ts-node prisma/seed-demo.ts
